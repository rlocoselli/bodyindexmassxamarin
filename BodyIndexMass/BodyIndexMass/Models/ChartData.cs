...
namespace BodyIndexMass.Models
{
    public class ChartDatabase
    {
        ...
        public ChartDatabase()
        {
            ...
            LineModel = CreateOneSeries();
        }

        private static List<BodyIndexMassEntity> GetData() {
            BodyIndexMassData b = new BodyIndexMassData();
            return b.GetBodyAsync().Result;
        }
        public static PlotModel CreateOneSeries()
        {

            List<BodyIndexMassEntity> e = GetData();
            ...
            foreach(BodyIndexMassEntity item in e)
            {
                i++;
                s1.Points.Add(new DataPoint(i, item.Result));
            }
